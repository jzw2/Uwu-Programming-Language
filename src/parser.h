...
namespace naruto
{
	...
	public:
		...
		virtual llvm::Value * generate(std::vector<std::string> var_names, 
			llvm::Value * var_vals, std::string fn_name) { return nullptr; }
		virtual void print() {}
	...
	ASTBinOp(std::string o) : op(o) {}
		...
		virtual llvm::Value * generate(std::vector<std::string> var_names,
			llvm::Value * var_vals, std::string fn_name) override;
	...
	class CloneCall : public ASTExpr 
	{
		...
		ASTIden *thread_id;
	public:
		CloneCall(std::string s,  std::vector<llvm::Value*> ps, ASTIden *tid) { function_name = s; params = ps; thread_id = tid;}
